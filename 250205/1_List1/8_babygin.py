...
for k in range(0, len(arr), 3):
    scope = arr[k:k+3]
    # print(scope)
    # 3. 연속한 숫자이거나 똑같은 숫자인지 확인한다.
    if int(scope[0]) + 1 == int(scope[1]) and int(scope[1]) + 1 == int(scope[2]):
        result.append("run")
    if scope[0] == scope[1] and scope[1] == scope[2]:
        result.append("triplet")

# 4. 결과가 result나 triplet으로만 채워져있으면 babygin이다. 
valid = True
for x in result:
    if x not in ["run", "triplet"]:
        valid = False
        break
